      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {steps.map(({ num, icon: Icon, title, desc }) => (
            ...
            >
              <span className="absolute top-4 right-4 font-['Poppins'] text-6xl font-black text-gray-100 group-hover:text-[#E84D1C]/10 transition-colors select-none">
                {num}
              </span>
              <div className="w-14 h-14 rounded-xl bg-[#E84D1C]/10 flex items-center justify-center mb-5">
                <Icon className="w-7 h-7 text-[#E84D1C]" />
              </div>
              <h3 className="font-['Poppins'] text-xl font-semibold text-[#2B3A67] mb-2">{title}</h3>
