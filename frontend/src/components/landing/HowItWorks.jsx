    ...
    <section id="comment-ca-marche" data-testid="how-it-works-section" className="bg-[#F3F4F6] py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 md:mb-16">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#FF5C00] mb-3">Simple et rapide</p>
          <h2 className="font-['Outfit'] text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-[#1E2A44]">
            Comment ca marche ?
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {steps.map(({ num, icon: Icon, title, desc }) => (
            ...
            >
              <span className="absolute top-4 right-4 font-['Outfit'] text-6xl font-black text-gray-100 group-hover:text-[#FF5C00]/10 transition-colors select-none">
                {num}
              </span>
              <div className="w-14 h-14 rounded-xl bg-[#FF5C00]/10 flex items-center justify-center mb-5">
                <Icon className="w-7 h-7 text-[#FF5C00]" />
              </div>
              <h3 className="font-['Outfit'] text-xl font-semibold text-[#1E2A44] mb-2">{title}</h3>
